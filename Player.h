...
class Player
{
	...
	/*me*/  int lastDiceRoll;  // store the last dice roll
	/*me*/  bool isTurnCanceled;    //for card 8 effect 
									//initialized to 'false' in the constructor's initialization list
public:
	...
	int GetBetweenCellsNum(int target);
	void AppendPlayerInfo(string& playersInfo) const; // Appends player's info to the input string, 
	// for example: P0(wallet, turnCount)
//me: added this
	void DecrementWallet(int amount);
	void IncrementWallet(int amount);
	/*me*/ void SetLastDiceRoll(int roll);
	/*me*/ int GetLastDiceRoll() const;
	/*me*/ void SetTurnCount(int x);
	/*me*/ void DecTurnCount();
	void SetTurnCanceled(bool c);
	bool GetTurnCanceled() const;
};
