...
int Output::GetCellStartY(const CellPosition& cellPos) const
{
	...
	return 55 + (cellPos.VCell() * 55); // this line should be changed with your implementation
}
...
void Output::CreatePlayModeToolBar() const
{
	...
	//MenuItemImages[ITM_EXIT] = "images\\Menu_Exit_Play.jpg";
	// 
...
///TODO: Draw the cell number in the x and y location
	...
	if (cardNum >= 1 && cardNum <= 9) // Note: cardNum 1 => 9
...
{
	if (toCell.GetCellNum() <= fromCell.GetCellNum() || toCell.HCell() != fromCell.HCell() || fromCell.GetCellNum() >= 89)
	{
		PrintMessage("Check ladder specifications");
...
///TODO: Set pen color and width using the appropriate variables of UI_Info object (UI)
	...
	for (int i = 0;i < fromCell.VCell() - toCell.VCell();i = i + 1)
		pWind->DrawLine(x12, toStartY + 55 + i * 55, x34, toStartY + 55 + i * 55);
...
{
	if (toCell.GetCellNum() >= fromCell.GetCellNum() || toCell.HCell() != fromCell.HCell() || fromCell.GetCellNum() <= 11) {
		return;
		///TODO: Validate the fromCell and toCell (Must be Vertical and toCell is below fromCell otherwise do NOT draw)
		PrintMessage("Check snake specifications");
	}
	else
	{
		...
		int f[] = { x12,x12 - xChange,x12,x12 + xChange };
		int g[] = { y1 - yChange,y1,y1 + yChange,y1 };


		///TODO: Draw the Polygon (diamond) representing the Snake's Head
		//       Check the snakes drawn in the project document and draw it the same way
		pWind->DrawPolygon(f, g, 4, FILLED);
	}
